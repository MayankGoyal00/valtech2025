...
class LambdaExpressionsTest {
	...
	void test() {
		...
		cars.add(new Car("Benx","EClass","220D",2024,5));
		cars.add(Car.builder().make("honda").model("City").variant("ZX").year(2024).version(6).build());
		cars.add(Car.builder().make("honda").model("JAZZ").variant("ZX").year(2024).version(7).build());
		...
		//myNewCars.parallelStream();//new tread 
