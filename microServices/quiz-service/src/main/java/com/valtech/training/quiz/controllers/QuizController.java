...
public class QuizController {
    ...
    @PostMapping("/create")
    public QuizVO createQuiz(@RequestParam String topic, @RequestParam int numQ,@RequestParam String title) {
    	return quizService.createQuiz(topic, numQ, title);
    }
 
    @PostMapping("/{quizId}/submit")
    public int calculateResult(@PathVariable Long quizId, @RequestBody List<Response> responses, @RequestParam String userName) {
    	return quizService.calculateResult(quizId, responses, userName);
    }
 
    @GetMapping("/{quizId}/set")
